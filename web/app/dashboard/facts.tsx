...
export function Facts({ ledgers }: { ledgers: string[] }) {
  ...
  return (
    <div>
      ...
      {/* Keyed by ledger: picking another one remounts, so every piece of
          browsing state — the pattern, the page, the snapshot — is discarded
          rather than carefully reset. */}
      {ledger && <Browser key={ledger} ledger={ledger} />}
    </div>
  )
}

function Browser({ ledger }: { ledger: string }) {
  ...
  const run = useCallback(
    async (reopen: boolean) => {
      ...
      setError(null)
      try {
        // Re-asking at the same name gives the same facts, which is the point.
        // Advancing is a separate, deliberate act.
  ...
  // Landing on a ledger opens it and asks it everything. Nobody wants to press
  // a button to see what is in the thing they just clicked.
  useEffect(() => {
    let live = true

    open([ledger])
      .then(async (at) => ({ at, found: await ask(at, {}) }))
      .then(({ at, found }) => {
        if (!live) return
        setName(at)
        setFacts(found)
    ...
    <>
      ...
      <div className="mt-10">
        ...
        <p className="mb-4 max-w-2xl text-xs leading-relaxed text-muted-foreground">
          an empty field is a wildcard; a filled one matches exactly. a value
          that parses as json is sent as json —{" "}
          <code className="font-mono">42</code> is the number,{" "}
          <code className="font-mono">&quot;42&quot;</code> is the string.
        </p>
      ...
      {error && (
        <RefusalNote error={error} className="mt-8" retry={() => run(true)} />
      )}
        ...
        <div className="mt-10">
          ...
          {narrowed.length === 0 ? (
            <p className="mt-8 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              ...
              not an error — the snapshot is real and it holds no such fact.
            </p>
          ) : (
