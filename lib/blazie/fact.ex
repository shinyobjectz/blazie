defmodule Blazie.Fact do
  ...
  **An append-only store must read every shape it has ever written, forever.**
  ...
  ## The module's own name is part of the shape

  `term_to_binary` stores which struct a map is, so renaming the project renamed
  the row. Every fact written as `LazyRiver.Fact` is still on disk under that
  name and always will be — the atom below is a literal on purpose, and must
  never be "fixed" to match the current module.

  Two renames have happened, and a record can predate either or both: the third
  slot went from `answer` to `value`, and the module from `LazyRiver.Fact` to
  `Blazie.Fact`. All four combinations exist somewhere, so this reads the shape
  rather than guessing the era.
  """

  # Deliberately a literal. This name no longer exists as a module and never
  # will again, which is exactly why a fact written under it needs it spelled
  # out here rather than referenced.
  @renamed_from :"Elixir.LazyRiver.Fact"
  @shapes [__MODULE__, @renamed_from]

  @spec from_stored(term()) :: term()
  def from_stored(%__MODULE__{value: _} = fact), do: fact

  def from_stored(%{__struct__: shape} = older) when shape in @shapes do
    %__MODULE__{
      id: older.id,
      attribute: older.attribute,
      value: stored_value(older),
      tx: older.tx,
      by: Map.get(older, :by)
    }
  end
  ...
  # Matched rather than fetched with a default, because `nil` and `false` are
  # values a fact may legitimately hold.
  defp stored_value(%{value: value}), do: value
  defp stored_value(%{answer: value}), do: value
