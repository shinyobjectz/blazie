defmodule Blazie.Job do
  ...
  only what `Blazie.Lua.capabilities/1` names for the kind. Lua on the BEAM is
  the one guest runtime (the wasm lane was retired — docs/storage-plan.md,
  LT3), so the network decision is one function every guest reads, not two fences that must agree — and this
