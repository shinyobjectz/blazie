defmodule Blazie.Cluster do
  ...
  Two nodes both open world `tenant-7`. Each starts its own process, each
  ...
  So a world's name is claimed across the cluster when it opens. A claim
  ...
  @doc "Which node owns this world, or nil if nobody has claimed it."
  ...
  @doc "Is this world ours to run?"
  @spec owned_here?(name()) :: boolean()
  def owned_here?(name), do: owner(name) == node()

  @doc false
  # Exposed so a test can stand in for another node's world process.
  ...
  @doc "Why a world somebody else holds cannot be opened here."
  ...
  defp held_elsewhere(name) do
    %{
      ...
      repair:
        "#{inspect(name)} is already open on #{inspect(owner(name))}. A world has one owner: " <>
