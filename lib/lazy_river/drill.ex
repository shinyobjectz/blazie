defmodule LazyRiver.Drill do
  @moduledoc """
  Giving the facts back, on a cadence — which makes a restore a **job** too.

  `LazyRiver.Backup` proves that bytes leave. Nothing it does proves they can
  ...
  Only that ledger's segments cross the wire. `LazyRiver.Drill.Sample` is a
  ...
  **A run that finds nothing to drill is not a failure.** It records
  ...
  alias LazyRiver.{Attribute, Backup, Job, Ledger, Snapshot, Store}
  ...
  rotation. A snapshot with none of those facts in it picks whatever is first.
      ...
      run(Snapshot.open([drill_ledger]),
          target: {Backup.Target.S3, bucket: "lazyriver", ...},
          scratch_dir: "/tmp",
  ...
  defp target(opts) do
    case Keyword.get(opts, :target) || Application.get_env(:lazy_river, :backup_target) do
      {module, topts} ->
        {module, topts}
  ...
  defp ceiling(opts) do
    Keyword.get(opts, :max_bytes) ||
      Application.get_env(:lazy_river, :drill_max_bytes) ||
      @default_max_bytes
  end
  ...
  defp scratch_dir(opts) do
    dir =
      Keyword.get(opts, :scratch_dir) ||
        Application.get_env(:lazy_river, :drill_dir) ||
        System.tmp_dir!()

    live =
      [:ledger_dir, :key_dir]
      |> Enum.map(&Application.get_env(:lazy_river, &1))
...
defmodule LazyRiver.Drill.Sample do
  @moduledoc """
  One ledger's worth of a backup, and nothing else.

  A `LazyRiver.Backup.Target` that wraps the configured one and hides every key
