...
<!-- GENERATED by monty gen sync instruments=sha256:bd2a38cad6334739 -->
...
| **generative programming** | `gen` | adopted · noun | Writing programs whose stochastic parts are constrained by verification rather than by hope. A generative program contains calls to a model; generative programming is the discipline of keeping the uncertainty those calls introduce from accumulating — every call circumscribed by a requirement, no unverified output allowed to land, and a failed check carrying its reason into the repair. Here it extends one step further than the term usually reaches: a program's SOURCE may itself be generated, verified against examples before adoption, and stored as facts — so a value can be traced to the program that computed it and to the thing that wrote that program, in one structure. | Would the output be trusted if nothing checked it? If yes it is prompting; if no, and something does check, it is generative programming. |
...
| **cluster** | `clu` | core · noun | A running blazie, reachable at an address, holding worlds. One or more nodes that agree on who owns which world — from the outside, the thing you point a console or a client at; from the inside, the set of nodes sharing name claims. Both are the same set of machines, so both are this word. A cluster is what you OPEN, the same verb as a world and for the same reason: it is brought into being by being named, and opening one that already exists hands you the one that is there. | Could you point a client at it and ask a question? Then it is a cluster. If it is one machine inside one, it is a node, which is not a word here because nothing outside this repo has to know there is more than one. |
