...
<!-- GENERATED by monty gen sync instruments=sha256:86f5c0a3a538b00f -->
...
| **world** | `led` | core · noun | The append-only sequence facts go into, and the boundary that owns them. A ledger is readable, forkable and deletable on its own; a tenant is one or more ledgers. | Can it be read, forked and deleted without touching anything else? Then it is a ledger. |
