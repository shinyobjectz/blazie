defmodule Blazie.CompactionTest do
  ...
  defp write(world, range) do
    for n <- range, do: {:ok, _} = World.append(world, [{n, "height", n}])
  end

  defp time_appends(world, range) do
  ...
  describe "a checkpoint does not lose anything" do
    test "every fact is still there after reopening", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 20)
      write(world, 1..100)
      ...
      assert World.tx(reopened) == 100

      World.close(ctx.name)
    end

    test "a superseded fact survives, because nothing is rewritten", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 5)
      for v <- [1, 2, 3], do: {:ok, _} = World.append(world, [{42, "height", v}])
    ...
    test "an old transaction still answers what it always answered", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 10)
      write(world, 1..10)
  ...
  describe "keeping checkpoints does not cost more the longer you run" do
    ...
    test "a longer log does not cost more per append", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 50)

      write(world, 1..2_000)
      early = time_appends(world, 2_001..3_000)

      write(world, 3_001..12_000)
      late = time_appends(world, 12_001..13_000)
  ...
  describe "the order facts come back in" do
    test "replay is oldest first, whatever the store does internally", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 7)
      write(world, 1..40)
      :ok = World.close(ctx.name)
  ...
  describe "opening stops re-reading the whole log" do
    test "a checkpointed reopen reads far fewer records", ctx do
      world = on_disk(ctx.name, ctx.dir, checkpoint_every: 200)
      write(world, 1..500)
      :ok = World.close(ctx.name)

      reopened = on_disk(ctx.name, ctx.dir, checkpoint_every: 200)
      stats = World.store_stats(reopened)
